...
class Sequence(Dataset):
    def __init__(self, seq_path: Path, representation_type: RepresentationType, mode: str = 'test', delta_t_ms: int = 100,
                 num_bins: int = 4, num_frames: int = 2,transforms=[], name_idx=0, visualize=False, load_gt=False):
        assert num_bins >= 1
    ...
    def apply_transforms(self, event_representation, flow_gt=None):
        if 'random_crop' in self.transforms:
            i, j, h, w = RandomCrop.get_params(event_representation, output_size=self.transforms['random_crop'])
            event_representation = torchvision.transforms.functional.crop(event_representation, i, j, h, w)
            if flow_gt is not None:
                flow_gt = torchvision.transforms.functional.crop(flow_gt, i, j, h, w)
        if 'random_flip' in self.transforms and random.random() > 0.5:
            event_representation = torchvision.transforms.functional.hflip(event_representation)
            if flow_gt is not None:
                flow_gt = torchvision.transforms.functional.hflip(flow_gt)
                flow_gt[:, 0] = -flow_gt[:, 0]  # Flip x-component of flow
        if 'random_noise' in self.transforms:
            noise = torch.randn_like(event_representation) * self.transforms['random_noise']
            event_representation += noise
        return event_representation, flow_gt
    ...
    def get_data(self, index) -> Dict[str, any]:
        ...
        for i in range(start_index, end_index + 1):
            if i >= len(self.timestamps_flow):
                continue
            ts_start = self.timestamps_flow[i] - self.delta_t_us
            ts_end = self.timestamps_flow[i]
            event_data = self.event_slicer.get_events(ts_start, ts_end)
            if event_data is None:
                continue
            p, t, x, y = event_data['p'], event_data['t'], event_data['x'], event_data['y']
            xy_rect = self.rectify_events(x, y)
            x_rect, y_rect = xy_rect[:, 0], xy_rect[:, 1]
            event_representation = self.events_to_voxel_grid(p, t, x_rect, y_rect)
            event_representation = self.normalize_tensor(event_representation)
            event_volumes.append(event_representation)

        if len(event_volumes) < self.num_frames:
            raise ValueError("Insufficient frames")

        output = {
            'file_index': self.indices[end_index],
            'timestamp': self.timestamps_flow[end_index],
            'seq_name': self.seq_name,
            'save_submission': self.indices[end_index] in self.idx_to_visualize,
            'visualize': self.visualize_samples,
            'event_volume': torch.cat(event_volumes, dim=0),
            'name_map': self.name_idx
        }

        if self.load_gt:
            output['flow_gt'] = [torch.tensor(x) for x in self.load_flow(self.flow_png[end_index])]
            output['flow_gt'][0] = torch.moveaxis(output['flow_gt'][0], -1, 0)
            output['flow_gt'][1] = torch.unsqueeze(output['flow_gt'][1], 0)

        if self.transforms:
            event_representation, flow_gt_transformed = self.apply_transforms(output['event_volume'], output['flow_gt'][0] if self.load_gt else None)
            output['event_volume'] = event_representation
            if self.load_gt:
                output['flow_gt'][0] = flow_gt_transformed
    ...
    # def apply_augmentation(self, event_representation):
    #     # (C, H, W) -> (H, W, C)
    #     event_representation = event_representation.permute(1, 2, 0)  # (C, H, W) -> (H, W, C)
    #     event_representation = self.data_augmentation(event_representation)
    #     event_representation = event_representation.permute(2, 0, 1)  # (H, W, C) -> (C, H, W)
    ...
    def get_voxel_grid(self, idx):
        ...
        voxel_grid = self.events_to_voxel_grid(p, t, x_rect, y_rect)
        voxel_grid = self.normalize_tensor(voxel_grid)  # normalization
        return voxel_grid
...
class DatasetProvider:
    def __init__(self, dataset_path: Path, representation_type: RepresentationType, delta_t_ms: int = 100, num_bins=4,
                num_frames=2, config=None, visualize=False, transforms=None):
        ...
        for seq in seqs:
            train_sequences.append(Sequence(Path(train_path) / seq,
                                   ...
                                   num_frames=num_frames,
                                   load_gt=True, transforms=transforms))
...
# class DatasetProvider:
#     def __init__(self, dataset_path: Path, representation_type: RepresentationType, delta_t_ms: int = 100, num_bins=4,
#                 num_frames=2, config=None, visualize=False):
#         test_path = Path(os.path.join(dataset_path, 'test'))
#         train_path = Path(os.path.join(dataset_path, 'train'))
#         assert dataset_path.is_dir(), str(dataset_path)
#         assert test_path.is_dir(), str(test_path)
