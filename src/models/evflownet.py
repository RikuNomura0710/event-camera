...
class EVFlowNet(nn.Module):
    def __init__(self, args):
        ...
        self._args = args
        in_channels = 8 #args.num_frames * 4

        self.encoder1 = nn.Sequential(
            general_conv2d(in_channels=in_channels, out_channels=_BASE_CHANNELS, do_batch_norm=not self._args.no_batch_norm),
            nn.Dropout(_DROPOUT_PROB)
        )
        self.encoder2 = nn.Sequential(
            general_conv2d(in_channels=_BASE_CHANNELS, out_channels=2*_BASE_CHANNELS, do_batch_norm=not self._args.no_batch_norm),
            nn.Dropout(_DROPOUT_PROB)
        )
        self.encoder3 = nn.Sequential(
            general_conv2d(in_channels=2*_BASE_CHANNELS, out_channels=4*_BASE_CHANNELS, do_batch_norm=not self._args.no_batch_norm),
            nn.Dropout(_DROPOUT_PROB)
        )
        self.encoder4 = nn.Sequential(
            general_conv2d(in_channels=4*_BASE_CHANNELS, out_channels=8*_BASE_CHANNELS, do_batch_norm=not self._args.no_batch_norm),
            nn.Dropout(_DROPOUT_PROB)
        )
